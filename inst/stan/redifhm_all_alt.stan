...
parameters {
  // Fixed Effects
  row_vector[J] lambda_raw; // Loadings
...
transformed parameters {
  ...
  matrix[K, J] lambda_random_raw = random[, lamResNu_indices[1]];
  matrix[K, J] resid_random = random[, lamResNu_indices[2]];
  matrix[K, J] nu_random = random[, lamResNu_indices[3]];
  vector[K] eta_mean_raw = sum_coding ? eta_means_stz(eta_mean_s) : random[, total + 1];
  ...
  vector[N * save_scores] eta_raw; // Declare
  ...
  if(save_scores) { // Compute latent score.
    eta_raw = eta_mean_raw[group] + eta_z .* eta_sd[group];
  } else {
    // TODO: Optimization needed: Make marg_expect_uni return an array of vectors for mu.
    multi_normal_mu = marg_expect_uni(lambda_raw, nu, lambda_random_raw, nu_random, eta_mean_raw);
    multi_normal_sigma = marg_cov_uni(lambda_raw, resid_log, lambda_random_raw, resid_random, eta_sd);
  }
}
...
model {
  ...
  if(save_scores) { // Construct xhat and s_loghat.
    xhat = rep_matrix(nu, N) + eta_raw*lambda_raw; // Fixed effects
    s_loghat = rep_matrix(resid_log, N); // Fixed effects
    xhat += nu_random[group] + rep_matrix(eta_raw, J) .* lambda_random_raw[group];
  ...
  lambda_raw[1] ~ normal(.5, .2);
  lambda_raw[2:J] ~ std_normal();
  //// (Log) Residual SD
...
generated quantities {
  ...
  real lambda_1_sign = fabs(lambda_raw[1]) / lambda_raw[1];
  row_vector[J] lambda = lambda_raw * lambda_1_sign;
  matrix[K, J] lambda_random = lambda_random_raw * lambda_1_sign;
  vector[N*save_scores] eta = eta_raw * lambda_1_sign;
  vector[K] eta_mean = eta_mean_raw * lambda_1_sign;
}
